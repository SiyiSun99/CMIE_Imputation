...
imputation_methods <- list(
  ...
  "C18" = c('pmm', 'polyreg', 'polyreg', 'logreg', 'logreg', 'pmm', 'logreg', 'polyreg', 'polyreg', 
            'polyreg', 'pmm', 'pmm', 'logreg', 'logreg', 'logreg', 'logreg',rep("pmm", 12) )
)

# Modified function to process categorical variables with level reduction
process_categorical_vars <- function(df, max_levels = 45) {
  tryCatch({
    ...
    if (length(cat_indices) > 0) {
      for (col in names(df)[cat_indices]) {
        ...
        if (length(value_counts) > max_levels) {
          # Keep top categories and combine others into "Other"
          top_categories <- names(sort(value_counts, decreasing = TRUE)[1:max_levels])
          df[[col]][!(df[[col]] %in% top_categories)] <- "Other"
        }
...
# Custom quickpred function with minimum correlation threshold
...
# Function to check and log category counts
check_category_counts <- function(df) {
  cat_vars <- names(df)[grep("^cat_", names(df))]
  for (var in cat_vars) {
    n_levels <- length(levels(as.factor(df[[var]])))
    # cat(sprintf("Variable %s has %d categories\n", var, n_levels))
  }
}
    ...
    for (miss_rate in missList) {
      ...
      for (i in 1:sampleTime) {
        tryCatch({
          ...
          # Log category counts before processing
          # cat("Category counts before processing:\n")
          check_category_counts(mis_df)
          
          # Process categorical variables with max 45 levels
          mis_df <- process_categorical_vars(mis_df, max_levels = 45)
          
          # Log category counts after processing
          # cat("Category counts after processing:\n")
          check_category_counts(mis_df)
