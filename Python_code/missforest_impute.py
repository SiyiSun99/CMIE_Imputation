...
output_file = Path("/Users/siysun/Desktop/CMIE/CMIE_Project/imputation_times_missforest.csv")
time_records = []
...
for cohort in cohorts:
    ...
    for miss_method in tqdm(missing_methods):
        for miss_ratio in tqdm(missing_ratios):
            save_dir = f"data_missforest/{cohort}/{cohort}_all/{miss_method}/miss{miss_ratio}"
            ...
            for index_file in tqdm(range(SampleTime)):
                mask_path = base_path / f"data_miss_mask/{cohort}/{cohort}_all/{miss_method}/miss{miss_ratio}/{index_file}.csv"
            ...
            # Save results to CSV
            time_df = pd.DataFrame(time_records, columns=["Dataset", "Mechanism", "MissingRatio", "AvgTime"])
