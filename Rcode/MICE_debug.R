...
imputation_methods <- list(
  "C19" = c(
    "pmm", "polyreg", "polyreg", "logreg", "logreg", "pmm", "logreg", "polyreg", "polyreg",
...
for (cohort in cohorts) {
  ...
  for (method in missingMechanism) {
    ...
    for (miss_rate in missList) {
      ...
      for (i in 1:sampleTime) {
        tryCatch(
          ...
          error = function(e) {
            ...
            cat(sprintf(
              "Error processing %s, %s, miss rate %d, sample %d: %s\n",
      ...
      cat(sprintf(
        "Saved results for %s, %s, miss rate %d\n",
